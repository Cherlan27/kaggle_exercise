...
fare_train = preprocessing.StandardScaler().fit_transform(np.array(train_data['Fare']).reshape(-1,1)).reshape(1,-1)[0]
...
x_train = pd.concat([train_data["PassengerId"], train_data["Pclass"], age_train, train_data['Age'], train_data['SibSp'],
       train_data['Parch'], pd.DataFrame({"Fare": fare_train})], axis = 1)
...
fare_test = preprocessing.StandardScaler().fit_transform(np.array(test_data['Fare']).reshape(-1,1)).reshape(1,-1)[0]

x_test = pd.concat([test_data["PassengerId"], test_data["Pclass"], age_test, test_data['Age'], test_data['SibSp'],
       test_data['Parch'], pd.DataFrame({"Fare": fare_test})], axis = 1)


y_test = clf.predict(x_test)
prediction = pd.concat([test_data["PassengerId"], pd.DataFrame({"Survived": y_test})], axis = 1)
